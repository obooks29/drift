/**
 * Drift SDK client for the demo app.
 * Demonstrates real SDK usage end-to-end.
 */
import Drift from "@drift-ai/sdk";

export const drift = new Drift({
  apiKey:         process.env.DRIFT_API_KEY         ?? "drift_pk_demo_key",
  organizationId: process.env.DRIFT_ORG_ID          ?? "org_demo",
  apiUrl:         process.env.DRIFT_API_URL          ?? "http://localhost:4000/v1",
});
